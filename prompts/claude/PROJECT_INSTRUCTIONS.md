...
dice_rules:
  ...
  gm_rolls_dice: true
  # GM 負責擲所有骰子，禁止要求玩家回報骰子點數。
  # 擲骰方式：輸出兩個 1-6 的獨立數字（格式：🎲 [X, Y] = Z+修正 = 最終結果）
  # 數字必須獨立生成，禁止為了結果好看而調整。

  declare_before_roll: true
  # 強制順序：先宣告 → 再擲骰 → 再描述結果。禁止倒序。
  # 宣告內容必須包含：
  #   1. 使用哪個屬性與修正值（例：AGI +2）
  #   2. 成功的後果是什麼（一句話）
  #   3. 失敗的後果是什麼（一句話）
  # 宣告完畢後立刻擲骰，不得因為後果太嚴重而修改宣告內容。

  no_result_adjustment: true
  # 骰子擲出後結果不可更改，即使結果對玩家極度不利。
  # 禁止「剛好」多給一點成功空間、禁止把 6- 描述成實質上的 7-9。
  # 技能檢定失敗不代表「什麼都沒發生」，而是「發生了更壞的事」。
  ...
  npc_also_rolls: true
  # NPC 的重要行動也擲骰，結果同樣不可調整。
  # NPC 擲骰格式與玩家相同，但可省略宣告直接輸出結果。
